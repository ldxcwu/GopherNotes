...
<aside>
🍀 `range` 在迭代过程中，返回的是迭代值的拷贝；

</aside>

1. 如果迭代的元素内存占用较低（例如`[]int`）那么`for`和`range`性能几乎一样；
2. 如果迭代的元素内存占用较高（例如复杂`struct`）那么for的性能显著高于`range`；
3. 建议在迭代元素内存占用高时，使用`for`或者`range`只迭代下标，若想使用`range`同时迭代下标和值，建议将切片/数组的元素改为指针
